...
class ApiService {
  ...
  static String get membersUrl => '$baseUrl$endpoint';

  // GET members filtered by groupId (filter in app, not URL)
  Future<List<Member>> getMembers(String groupId) async {
    final response = await http.get(Uri.parse(membersUrl));

    if (response.statusCode == 200) {
      final List<dynamic> data = jsonDecode(response.body);
      final all = data.map((json) => Member.fromJson(json)).toList();
  ...
  Future<Member> getMember(String id) async {
    final response = await http.get(Uri.parse('$membersUrl/$id'));
  ...
  Future<void> deleteMember(String id) async {
    final response = await http.delete(Uri.parse('$membersUrl/$id'));
